...
class Graph:
    """
    Class which reads graph from file, provides basic information about the graph,
    like vertices, edges, adjacency matrix, etc.
    And also allows for basic transformations, like cyclic <-> wheel
    """

    class EulerType(Enum):
        """
        How much "euler" a graph is
        """
    ...
    @staticmethod
    def _get_edges(graph_as_str: str) -> List[Tuple[int, int]]:
        """
    ...
    def _get_vertices(self) -> List[int]:
        ...
        """
        return set(
            [first for first, _ in self.edges] + [second for _, second in self.edges]
        )
        ...
        for vertex in range(self.vertices_num):
            if all(
                (
                    ...
                    and self.adjacency_matrix[vertex][connection_vertex] == 1
                )
                or (
                    connection_vertex == vertex
                    and self.adjacency_matrix[vertex][connection_vertex] == 0
                )
    ...
    def is_wheel(self) -> bool:
        ...
        Note: Graph is considered as wheel if there is a center vertex
        which connects all other vertices and rest of vertices have degree 2
        """
    ...
    def is_euler(self) -> Graph.EulerType:
        """
        Indicates whether graph is euler (`Graph.EulerType.FULL'),
        half euler (`Graph.EulerType.HALF`) or not euler in any way (`Graph.EulerType.NONE`)
        Note: Graph is considered as euler if all degrees have even value, half euler
    ...
    def convert_cyclic_to_wheel(self) -> bool:
        ...
        vertices_to_connect = np.where(self.adjacency_matrix[central_vertex] == 1)[0]

        # Once vertex "0" is moved to the center, vertices that were connected
        # with it, have to connect with each other
        self.adjacency_matrix[vertices_to_connect[0]][vertices_to_connect[1]] = 1
        self.adjacency_matrix[vertices_to_connect[1]][vertices_to_connect[0]] = 1
    ...
    def convert_wheel_to_cyclic(self) -> bool:
        ...
        # Break all connections between center vertex and
        # rest of vertices (except vertices to connect)
        for vertex in range(self.vertices_num):
            if vertex == center_vertex:
    ...
    def color_greedy(self):
        ...
        for vertex in range(1, self.vertices_num):
            ...
            for connection in range(self.vertices_num):
                if (
                    self.adjacency_matrix[vertex][connection] == 1
                    and self.colors[connection] != -1
                ):
...
def task_1(graph_file: str) -> None:
    ...
    if graph.is_regular():
        print(f"Graf jest grafem regularnym, stopnia {list(graph.degrees.values())[0]}")
    else:
        print("Graf nie jest grafem regularnym")
...
def task_2(graph_file: str) -> None:
    ...
    if cyclic:
        ...
        graph.dump_graph(
            f"task_02_{graph_file.split('.')[0]}_to_wheel.jpg",
...
def task_3(graph_file: str) -> None:
    ...
    if wheel:
        ...
        graph.dump_graph(
            f"task_03_{graph_file.split('.')[0]}_to_cyclic.jpg",
...
def task_4(graph_file: str) -> None:
    ...
    print("Kolory przypisane do wierzchołków:")
    for vertex, color in enumerate(graph.colors):
        print(f"{vertex+1} - k{color}")
    graph.dump_graph(
        f"task_04_{graph_file.split('.')[0]}.jpg",
        f"task_04_{graph_file.split('.')[0]}.dot",
        True,
    )
...
def task_5(graph_file: str) -> None:
    ...
    if euler_type == Graph.EulerType.FULL:
        print(
            "Graf jest grafem eulerowskim (stopień każdego wierzchołka jest liczbą parzystą)"
        )
    elif euler_type == Graph.EulerType.HALF:
        print(
            "Graf jest grafem półeulerowskim (stopień dokładnie dwóch"
            "wierzchołków jest liczbą nieparzystą)"
        )
    else:
